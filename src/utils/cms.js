...
export const getContactPage = async () => {
    ...
    if (Token) {
        try {
            const res = await axios.get(`${import.meta.env.VITE_BASE_URL}/admin/contact-page`, {
                headers: {
                    'Authorization': `Bearer ${Token}`
                }
            });
...
export const updateContactPage = async (data) => {
    ...
    if (Token && data) {
        try {
            const res = await axios.put(`${import.meta.env.VITE_BASE_URL}/admin/contact-page`, data, {
                headers: {
                    'Authorization': `Bearer ${Token}`
                }
            });
            if (res?.data?.success == true) {
                toast.success(res?.data?.message || 'Contact page updated successfully');
                return res?.data
            }
        } catch (err) {
            toast.error(err.response?.data?.message);
