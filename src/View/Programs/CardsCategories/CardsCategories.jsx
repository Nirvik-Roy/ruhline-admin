...
const CardsCategories = () => {
    ...
    const filteredData = cardCategoryData?.filter((item) =>
    ...
    return (
        <>
            {loading && <Loaders />}
            {isModal && <AddCategoryModal addLoading={addLoading} setcardGame={setcardGame} cardGame={cardGame} postCategories={postCategories} setisModal={setisModal} />}
            {editModal && <EditCardCategory cardId={cardId} fetchCards={fetchCards} seteditModal={seteditModal} />}
            {deleleteModal && <DeleteModal loading={deleteLoading} details={'Do you really want to delete this category?'} title={'Delete Card Category'} setdeleteModal={setdeleteModal} onClick={deleteFunc} />}
